...
\textbf{Metric:} Discounted performance $J_\mu^{P,\pi} = \mu^\top V^{P,\pi}$ via exact evaluator; MC shown only as a diagnostic.\\[0.2cm]
\textbf{Observation:} Standard SPMI reaches $J\!\ge\!0.4$ at 534 iters and $0.5$ at 1490; federated runs plateau $\approx\!0.39$ and never hit $0.4$ within 2000 iters.\\[0.2cm]
\begin{center}
...
\textbf{Safety signal:} Dotted curves depict the theoretical bound $\hat{B}$; all methods keep it positive and shrinking ($\sim\!10^{-6}$ by the end), indicating conservative updates.\\[0.2cm]
\textbf{Conclusion:} On this track, federation does not reach the single-agent ceiling and the GP model chooser achieves comparable performance to the greedy case.\\[0.2cm]
\begin{center}
...
\begin{column}{0.32\textwidth}
\begin{block}{MC vs. True Performance}
\textbf{Alignment:} MC return estimates with mean gap $<10^{-3}$ to exact evaluator, thus MC tracks true with negligible bias.\\[0.2cm]
\textbf{Implication:} MC estimation enables practical deployment where exact evaluation is computationally prohibitive, scaling to larger state-action spaces without matrix inversions.\\[1.4cm]
\begin{center}
...
\begin{itemize}
    ...
    \item Safety guarantees impose limitations on performance an convergence
\end{itemize}
\end{block}
\end{column}
