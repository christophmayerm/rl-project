...
\textbf{Project core (extension):} Built on the original SPMI repo with a federated coordinator plus an optional GP-UCB model chooser. Agents share $\hat{Q},\hat{\delta},\hat{U}$ while keeping safety bounds intact.\\[0.2cm]
\textbf{Config focus:} Mirrors \texttt{config/config.yaml}: racetrack T1, $p_{\text{fail}}{=}0.07$, 4-agent F-SPMI, compared against standard SPMI and SA-PMI; current runs use the \textbf{GP model chooser} (latest data available).\\[0.2cm]
\textbf{Metric:} Ground-truth performance (\texttt{performance\_true}) for federated runs; standard/SA-PMI use their evaluation column (not strictly comparable). MC estimates are optimistic vs true.\\[0.2cm]
\begin{center}
...
\textbf{Safety signal:} Dotted curves show the bound term; federated SA-PMI keeps a tighter (lower) bound than single-agent SA-PMI while improving return, indicating conservative steps.\\[0.2cm]
\textbf{Federated lift (modest):} Returns climb faster than single-agent baselines, but asymptotic gains are small—benefit is earlier arrival, not higher ceiling.\\[0.2cm]
\begin{center}
...
\begin{column}{0.32\textwidth}
\begin{block}{Metric Gap: MC vs True}
\textbf{Gap highlighted:} Bar chart shows iterations to modest thresholds (0.10/0.15/0.20). MC reaches them far sooner than true performance, underscoring optimism in on-policy rollouts; standard uses a different eval metric.\\[0.2cm]
\begin{center}
\includegraphics[width=0.97\linewidth]{gp_vs_greedy.png}
\end{center}
\end{block}
\end{column}
